...
export const cloneCommand = new Command("clone")
  ...
  .argument(
    "<url>",
    "repository URL (e.g., https://localhost:3000/username/reponame)"
  )
  ...
  .action(async (url: string, directory?: string) => {
    try {
      ...
      if (!parsed) {
        ...
        console.log(
          chalk.yellow(
            "Expected format: https://localhost:3000/username/reponame"
          )
        );
        process.exit(1);
      }
      ...
      console.log(
        chalk.gray(`Fetching repository data for '${repositoryId}'...`)
      );
      ...
      const response = await axios.get(
        `${apiUrl}/repositories/${repositoryId}/clone`
      );
...
`;
      ...
      console.log(
        chalk.gray(`Recreating ${cloneData.commits.length} commits...`)
      );
      ...
      for (const commit of cloneData.commits) {
        ...
        commitContent += `author ${commit.author.name} <${
          commit.author.email
        }> ${Math.floor(new Date(commit.timestamp).getTime() / 1000)} +0000\n`;
        commitContent += `committer ${commit.committer.name} <${
        ...
        // Create tree object
        const treeEntries = commit.files
          .map((file) => {
      ...
      if (cloneData.headCommit) {
        ...
        const headCommit = cloneData.commits.find(
          (c) => c.sha === cloneData.headCommit
        );
        if (headCommit) {
      ...
      console.log(
        chalk.green(`✓ Repository cloned successfully into '${targetDir}'`)
      );
      console.log(chalk.gray(`  ${cloneData.commits.length} commits`));
      console.log(
        chalk.gray(
          `  ${cloneData.branches.length} branch${
            cloneData.branches.length !== 1 ? "es" : ""
          }`
        )
      );
      console.log(
        chalk.gray(
          `  HEAD is now at ${cloneData.headCommit?.substring(0, 7)} on ${
            cloneData.defaultBranch
          }`
        )
      );
    } catch (error: any) {
      if (error.response) {
        ...
        if (error.response.status === 404) {
          console.log(
            chalk.yellow(
              "Hint: Make sure the repository exists and you have access to it"
            )
          );
        }
      } else if (error.request) {
        console.error(
          chalk.red("Clone failed: Unable to connect to remote repository")
        );
        console.log(
          chalk.yellow(
            "Hint: Check your internet connection and repository URL"
          )
        );
      } else {
